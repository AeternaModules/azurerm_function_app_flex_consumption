variable "function_app_flex_consumptions" {
  description = <<EOT
Map of function_app_flex_consumptions, attributes below
Required:
    - location
    - name
    - resource_group_name
...
EOT
  ...
  type = map(object({
    ...
    maximum_instance_count                         = optional(number) # Default: 100
    instance_memory_in_mb                          = optional(number) # Default: 2048
    ...
    site_config = object({
      ...
      use_32_bit_worker                 = optional(bool)   # Default: false
